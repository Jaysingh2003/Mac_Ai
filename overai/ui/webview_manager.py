...
from WebKit import (
    ...
    WKPermissionDecision, WKMediaCaptureType
)
from AppKit import (
...
class WebViewManager(NSObject):
    ...
    def create_webview(self, frame) -> WKWebView:
        ...
        # Set self as UI delegate to intercept mic permission requests
        self._web_view.setUIDelegate_(self)
    ...
    def _setup_message_handler(self, config: WKWebViewConfiguration):
        ...
        # Audio capture handler (mic + system audio, no popup)
        controller.addScriptMessageHandler_name_(self, "audioCapture")
        ...
        script = """
            (function() {
                ...
                // Override getUserMedia to capture mic + system audio via Python
                // This replaces the browser's default mic-only capture silently
                const _origGetUserMedia = navigator.mediaDevices.getUserMedia.bind(navigator.mediaDevices);
                navigator.mediaDevices.getUserMedia = function(constraints) {
                    if (constraints && constraints.audio) {
                        // Signal Python to start mixed capture
                        if (window.webkit && window.webkit.messageHandlers && window.webkit.messageHandlers.audioCapture) {
                            window.webkit.messageHandlers.audioCapture.postMessage({ action: 'start' });
                        }
                        // Still return a real mic stream so the page UI works normally
                        return _origGetUserMedia(constraints);
                    }
                    return _origGetUserMedia(constraints);
                };

                // Expose stop function for when recording ends
                window._overaiStopAudio = function() {
                    if (window.webkit && window.webkit.messageHandlers && window.webkit.messageHandlers.audioCapture) {
                        window.webkit.messageHandlers.audioCapture.postMessage({ action: 'stop' });
                    }
                };

                // Receive mixed WAV base64 from Python
                window.receiveAudio = function(wav_b64) {
                    // Available for custom use - web services handle their own audio
                    console.log('Mixed audio ready, size:', wav_b64.length);
                };
            })();
        """
    ...
    # MARK: - Audio capture (mic + system audio mixed)

    def _handle_audio_capture_message(self, data):
        """Handle audioCapture messages from JS: start / stop."""
        from ..utils.audio_capture import get_audio_capture
        action = str(data.get('action', ''))

        if action == 'start':
            def on_complete(wav_b64: str):
                if wav_b64:
                    self._run_js(f"window.receiveAudio('{wav_b64}')")
                else:
                    self._run_js("window.receiveAudio('')")
            get_audio_capture().start(on_complete)
    ...
    def userContentController_didReceiveScriptMessage_(self, controller, message):
        ...
        elif message.name() == "audioCapture":
            self._handle_audio_capture_message(message.body())
