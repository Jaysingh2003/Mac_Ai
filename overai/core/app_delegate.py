...
class AppDelegate(NSObject):
    ...
    def _on_onboarding_complete(self, success: bool):
        """Called when accessibility onboarding completes."""
        if success:
            logger.info("Onboarding complete - permission granted")
        else:
            logger.warning("Onboarding closed without permission")
        
        # Check if BlackHole onboarding is needed (shown once)
        from ..utils.blackhole_onboarding import should_show_blackhole_onboarding, show_blackhole_onboarding
        if should_show_blackhole_onboarding():
            logger.info("Showing BlackHole audio setup onboarding")
            self._blackhole_onboarding = show_blackhole_onboarding(self._on_blackhole_complete)
        else:
            self._complete_setup()

    def _on_blackhole_complete(self, installed: bool):
        """Called when BlackHole onboarding completes."""
        if installed:
            logger.info("BlackHole installed — system audio capture enabled")
        else:
            logger.info("BlackHole skipped — mic-only audio capture")
        self._complete_setup()
