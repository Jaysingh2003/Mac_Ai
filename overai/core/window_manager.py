...
from Quartz import CGEventTapEnable, CGShieldingWindowLevel
import platform
...
class WindowManager(NSObject):
    ...
    def createWindow(self) -> OverAI_OverlayPanel:
        ...
        # Apply all screen capture protection
        self._apply_capture_protection(window)
    ...
    def _apply_capture_protection(self, window):
        """Apply all layers of screen capture protection.
        
        - NSWindowSharingNone: blocks CGWindowListCreateImage (OBS, screenshots)
        - CGShieldingWindowLevel: above display capture layer (Zoom/Teams)
        - NSWindowCollectionBehaviorIgnoresCycle: excluded from window enumeration
        - ScreenCaptureKit exclusion: excluded from SCStream captures (macOS 12.3+)
        """
        # Layer 1: Block CGWindowListCreateImage API
        window.setSharingType_(NSWindowSharingNone)
        
        # Layer 2: Place above screen capture layer
        try:
            shield_level = CGShieldingWindowLevel()
        except Exception:
            shield_level = NSFloatingWindowLevel + 1
        window.setLevel_(shield_level)
        
        # Layer 3: Exclude from window enumeration
        ...
        # Layer 4: ScreenCaptureKit exclusion (macOS 12.3+)
        self._apply_sck_exclusion(window)

        logger.debug("Screen capture protection applied")

    def _apply_sck_exclusion(self, window):
        """Exclude window from ScreenCaptureKit streams (macOS 12.3+)."""
        try:
            ...
            def _exclude(content):
                try:
                    ...
                    # Build filter excluding our window
                    display = content.displays()[0] if content.displays() else None
                    if display is None:
                        return

                    content_filter = SCK.SCContentFilter.alloc()
                    content_filter = content_filter.initWithDisplay_excludingWindows_(
                        display, [target]
                    )
                    logger.debug(f"SCK exclusion applied for windowID {window_id}")
                except Exception as e:
                    logger.debug(f"SCK exclusion inner error: {e}")

            SCK.SCShareableContent.getShareableContentWithCompletionHandler_(
                lambda content, err: _exclude(content)
            )

        except ImportError:
            logger.debug("ScreenCaptureKit not available")
        except Exception as e:
            logger.debug(f"SCK exclusion error: {e}")
    ...
    def showWindow(self):
        ...
        # Re-enforce capture protection every time window is shown
        # (some apps re-enumerate windows after they appear)
        self._apply_capture_protection(self._window)
